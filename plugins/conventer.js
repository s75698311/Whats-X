    ...
    Asena.addCommand({pattern: 'vsticker$', desc: Lang.ANİM_STİCK, fromMe: true}, (async (message, match) => {
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                const saving = data.pipe(fs.createWriteStream('/root/Whats-X/stweb.mp4'))
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/Whats-X/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: 'Feito com Whats-X', quoted: message.data })
                    if (fs.existsSync(savedFilename)) fs.unlinkSync(savedFilename)
                    if (fs.existsSync('/root/Whats-X/stweb.mp4')) fs.unlinkSync('/root/Whats-X/stweb.mp4')
                })
            })
        })
    }));
