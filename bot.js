...
// Sql
const WhatsAsenaDB = config.DATABASE.define('Whats-X', {
    info: {
...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        ...
        await axios.get('https://gist.github.com/xmtscf/bfda43b2a5176ad2027553abb2d745c0/raw').then(async (ann) => {
    ...
    setInterval(async () => { 
        if (biography_var == 'true') {
            if (conn.user.jid.startsWith('90')) { // Turkey
                ...
                var ov_time = new Date().toLocaleString('BR', { timeZone: 'America/Sao_Paulo' }).split(' ')[1]
                const get_localized_date = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
                var utch = new Date().toLocaleDateString(config.LANG, get_localized_date)
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time + '\n\nWhats-X'
                await conn.setStatus(biography)
    ...
    var insult = await axios.get('https://gist.githubusercontent.com/xmtscf/47ce36bbf371730ce8e7ec89979a249a/raw')
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('Whats')}${chalk.blue.bold('-X')}
${chalk.white.bold('Versão:')} ${chalk.red.bold(config.VERSION)}

${chalk.blue.italic('⏳ Conectando ao WhatsApp... Aguarde.')}`);
    });
    conn.on('credentials-updated', async () => {
        console.log(
            chalk.green.bold('✔️ Login bem-sucedido!')
        );
        console.log(
        ...
        console.log(chalk.bgGreen('Whats-X' + afwhasena));
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Whats-X modo Público!*\n\n_Não teste plug-ins aqui. Este é o seu número de LOG._\n_Você pode testar comandos em qualquer bate-papo :)_\n\n*Whats-X em modo particular. Para mudar, use* _.setvar WORK_TYPE:private_\n\n*Obrigado por usar Whats-X*', MessageType.text);
                }               
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Whats-X modo Privado!*\n\n_Não teste plug-ins aqui. Este é o seu número de LOG._\n_Você pode testar comandos em qualquer bate-papo :)_\n\n*Whats-X em modo particular. Para mudar, use* _.setvar WORK_TYPE:public_\n\n*Obrigado por usar Whats-X*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(
                    conn.user.jid,
                    '_Parece que você deseja alternar para o modo privado! Desculpe seu_ *WORK_TYPE* _A chave está incorreta!_ \n_Não se preocupe! Estou tentando encontrar o caminho certo para você.._', MessageType.text
                );
                await heroku.patch(baseURI + '/config-vars', {
                    body: {
                        ['WORK_TYPE']: 'private'
    ...
    conn.on('message-new', async msg => {
        ...
        if (config.SUPPORT2 == '0') {     
            var tsup = config.SUPPORT2.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (config.SUPPORT3 == '0') {     
    ...
    try {
        ...
        if (!nodb) {
            console.log(chalk.red.bold('Atualizando sua string de versão antiga...'))
            conn.loadAuthInfo(Session.deCrypt(config.SESSION)); 
            try {
                await conn.connect();
            } catch {
